...
// (fully-qualified table names) — the old `?schema=inventory` connection-string
// param is Prisma-native-engine-only, and Neon's pooled endpoint rejects
// `search_path` as a startup parameter.
const adapter = new PrismaNeon({ connectionString: process.env.INVENTORY_DATABASE_URL });
