...
npm run dev -w services/purchasing-service           # http://localhost:4003
```
...
- The gateway does not yet proxy to this service (separate follow-up task) — hit it directly at `:4003` with `x-user-id` / `x-user-roles` headers, same as inventory-service.
