...
export function SpendReportTable({ lines }: { lines: SpendReportLine[] }) {
  if (lines.length === 0) {
    return <p className="text-sm text-zinc-500">No spend recorded in this date range.</p>;
  }
  ...
  return (
    <div className="overflow-x-auto rounded border border-zinc-200 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody>
          {lines.map((line) => (
            <tr key={line.groupKey} className="border-t border-zinc-100 dark:border-zinc-800">
