...
export default async function ReceivePurchaseOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex animate-fade-in-up flex-col gap-6">
      <div>
        <Link href={`/purchase-orders/${po.id}`} className="text-sm text-brand transition-colors hover:underline">
          ← {po.poNumber}
        </Link>
        <h1 className="mt-2 text-2xl font-semibold text-stone-900 dark:text-stone-50">Receive goods</h1>
        <p className="text-sm text-stone-500">PRD §3.4</p>
      </div>
