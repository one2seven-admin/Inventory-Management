...
export default async function RecipeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex animate-fade-in-up flex-col gap-6">
      <div>
        <Link href="/recipes" className="text-sm font-medium text-brand transition-colors duration-150 hover:text-brand-hover hover:underline">
          ← Recipes
        </Link>
        <h1 className="mt-2 text-2xl font-bold tracking-tight text-stone-900 dark:text-stone-50">{recipe.name}</h1>
